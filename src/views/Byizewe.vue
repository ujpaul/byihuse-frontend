<template>
  <div class="emb-contact-wrap">
    ...
    <div class="delivery" style="margin: 20px 10px 30px 10px">
      <v-btn
        style="min-width: 45%; height: 60px"
        @click="registerAsLawyer"
        :class="{ accent: attached }"
        >{{ $t("message.RegisterAsTechnician") }}
      </v-btn>
      <v-btn
        style="min-width: 54%; height: 60px"
        id="premium"
        @click="findLawyers"
        :class="{ accent: !attached }"
        >{{ $t("message.FindTechinician") }}
      </v-btn>
    </div>
          ...
          <div class="text-center" style="margin-left: 20px">
            <v-card-text v-if="registerLawyer">
              <v-form ref="form" v-model="valid">
                ...
                ></v-text-field>
                <v-text-field
                  @keyup="checkInvalidId"
                  :placeholder="$t('message.idNumber')"
                  v-model="idnbr"
                  :rules="inputRules.basictextRules"
                  type="number"
                ></v-text-field>
                <small style="color: red" v-if="invalidIdNumber"
                ...
                <v-select
                  :items="types"
                  :label="$t('message.typeOfTechnician')"
                  v-model="type"
                ></v-select>
                <v-select
                  :items="levels"
                ...
                ></v-file-input>
                <v-file-input
                  :label="$t('message.Diploma')"
                  v-model="diploma"
                  accept="application/pdf"
                ></v-file-input>
                <v-file-input
                  :label="$t('message.PreviousWork')"
                  v-model="previous"
                  accept="image/*"
                ></v-file-input>
                <v-file-input
                  :label="$t('message.Photo')"
                  v-model="passport"
                  accept="image/*"
                ></v-file-input>
          ...
          <div class="text-center" style="margin-left: 20px">
            <v-card-text v-if="requestTechnician">
              <v-form ref="form" v-model="valid">
                ...
                <v-select
                  :items="experience"
                  :label="$t('message.ExperienceNeeded')"
                  v-model="exp"
                ></v-select>
                <v-select
                  :items="places"
                  :label="$t('message.Workplace')"
                  v-model="place"
                  @change="changePlace"
                ></v-select>
                <v-textarea
                  :label="$t('message.WorkDetails')"
                  v-model="details"
                ></v-textarea>
  ...
  data() {
    return {
      ...
      types: [
        ...
        {
          text: "Industrial Machine Operator",
          value: "industrial machine operator",
        },
  ...
  methods: {
    changeTechnician() {
      ...
      if (this.type === "mechanical") {
        ...
        this.specialist = [
          {
            text: "Smoak detector technician",
            value: "Smoak detector technician",
          },
          ...
          {
            text: "Network cable technician",
            value: "Network cable technician",
          },
          {
            text: "High volatage cable technician",
            value: "High volatage cable technician",
          },
        ...
        this.specialist = [
          ...
          {
            text: "Installation of w.c equipment's technician",
            value: "Installation of w.c equipment's technician",
          },
          {
            text: "STP(sewage treatment plan) technician",
            value: "sewage treatment plan technician",
          },
          {
            text: "WWTP(Waste Water Treatment plan) technician",
            value: "Waste Water Treatment plan technician",
          },
        ];
      } else {
        this.appear = false;
